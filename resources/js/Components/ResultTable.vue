...
<template>
      ...
      <table class="table-auto w-full text-left whitespace-no-wrap">
          <thead>
              <tr>
                  ...
                  <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tr rounded-bl">構成比</th>
              </tr>
          </thead>
          <tbody>
              <tr v-for="item in data.data" :key="item.date">
                  <td class="border-b-2 border-gray-200 px-4 py-3">{{ item.decile }}</td>
                  <td class="border-b-2 border-gray-200 px-4 py-3">{{ item.average.replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</td>
                  <td class="border-b-2 border-gray-200 px-4 py-3">{{ item.totalPerGroup.replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</td>
  ...
  <div v-if="data.type === 'rfm'" class="lg:w-2/3 w-full mx-auto overflow-auto">
    <h2 class="text-center text-2xl">RFMの分析結果</h2>
    <div class="my-4 text-center">合計：{{ data.totals }}人</div>
    <h3 class="text-center text-xl my-4">RFMランク毎の人数</h3>
    <table class="table-auto w-full text-left whitespace-no-wrap">
        <thead>
            <tr>
                <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">ランク</th>
                ...
                <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tr rounded-bl">M</th>
            </tr>
        </thead>
        <tbody>
    ...
    <h3 class="text-center text-xl my-4">RとFの集計表</h3>
    <table class="table-auto w-full text-left whitespace-no-wrap">
        <thead>
            <tr>
                ...
                <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tr rounded-bl">f_1</th>
            </tr>
        </thead>
        <tbody>
